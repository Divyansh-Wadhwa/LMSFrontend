...
const ClientSettingsPage = () => {
  ...
  const content: Record<string, React.ReactNode> = {
    General: (
      <div className="space-y-5">
        <Card title="Institution Profile">
          <Field label="Institution Name"><Input defaultValue="Tech University" /></Field>
          <Field label="Admin Email"><Input type="email" defaultValue="admin@techuni.edu" /></Field>
    ...
    'Platform Config': (
      <div className="space-y-5">
        <Card title="Batch & Student Limits">
          <Field label="Max Students per Batch"><Input type="number" defaultValue="60" /></Field>
          <Field label="Max Batches"><Input type="number" defaultValue="10" /></Field>
          <Toggle label="Allow students to self-enroll" defaultChecked={false} />
          <Toggle label="Send welcome email on student import" defaultChecked={true} />
          <SaveBtn />
        </Card>
        <Card title="Content Settings">
          <Field label="Max File Upload Size (MB)"><Input type="number" defaultValue="50" /></Field>
          <Toggle label="Allow students to download resources" defaultChecked={true} />
          <SaveBtn />
        </Card>
      </div>
    ),
    ...
    <PermissionGuard requiredRoles={['super_admin', 'client_admin']}>
      <div className="space-y-6">
        <div className="bg-white rounded-xl shadow-sm p-6">
          ...
          <p className="text-gray-500 text-sm mt-1">Manage your institution preferences and account settings</p>
        </div>
            ...
            <nav className="flex gap-1 -mb-px">
              {TABS.map(tab => (
                <button key={tab} onClick={() => setActiveTab(tab)}
                  className={`py-4 px-4 text-sm font-medium border-b-2 whitespace-nowrap transition-colors ${activeTab === tab ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}>
                  {tab}
                </button>
              ))}
            </nav>
