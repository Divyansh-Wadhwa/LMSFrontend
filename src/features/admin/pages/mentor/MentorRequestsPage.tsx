              ...
              {filtered.map(req => (
                <div key={req.id} className="p-5 hover:bg-gray-50 transition-colors">
                  <div className="flex items-start justify-between gap-4">
                    ...
                    {req.status === 'approved' && (
                      <button
                        onClick={() => navigate(`/admin/mentor-preview/${req.id}`)}
                        className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-emerald-600 border border-emerald-200 rounded-lg hover:bg-emerald-50 transition-colors flex-shrink-0">
